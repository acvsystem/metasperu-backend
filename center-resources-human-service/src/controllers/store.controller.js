...
export const storeController = {
    ...
    getOneSearchScheduleStore: async (req, res) => {
        const { old_code_store, code_store, range_days } = req.body;
        ...
        try {
            ...
            for (const item of datos) {
                ...
                const [resCab] = await connection.execute(
                    `INSERT INTO tb_horario_property (FECHA, RANGO_DIAS, CARGO, CODIGO_TIENDA, ESTADO, DATETIME) 
                 VALUES (?, ?, ?, ?, 1, NOW())`,
                    [n(fechaCabecera), n(rango), n(item.cargo), n(codigoTienda)]
                );
                ...
                for (const d of item.dias) {
                    ...
                    if ((d.notasDia || []).length) {
                        for (const nt of d.notasDia) {
                            // Solo insertamos si hay texto real
                            await connection.execute(
                                `INSERT INTO tb_observacion (ID_OBS_HORARIO, ID_OBS_DIAS, CODIGO_TIENDA, NOMBRE_COMPLETO, NRO_DOCUMENTO, OBSERVACION, FECHA_REGISTRO) 
                             VALUES (?, ?, ?, ?, ?, ?, ?)`,
                                [idHorario, mappingDias[d.id], n(codigoTienda), n(nt.nombre_completo), n(nt.nro_documento), n(nt.observacion), n(nt.fecha_registro)]
                            );
                        }
                    }
                }
    ...
    postCreateBallotEmployes: async (req, res) => {
        ...
        try {
            ...
            await connection.rollback();
            const code = error.message.includes("Ya existe") ? 200 : 500;
    ...
    postApprovalHoursWorksEmployes: async (req, res) => {
        ...
        try {
            ...
            // Si es aprobado: aprobado=1, rechazado=0. Si no: aprobado=0, rechazado=1
            const values_hrx = [aprobado ? 'correcto' : 'rechazado', aprobado ? 1 : 0, aprobado ? 0 : 1, id_hrx];
...
const procesarYRegistrarHoras = async (listaRegistros) => {
    ...
    for (const [fecha, data] of Object.entries(resumenFullTime)) {
        ...
        const nivel = await validarNivelAutorizar(fecha, data.nroDocumento);
